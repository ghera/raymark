...
raylib_ref = "tags/5.5.zip"
-- esempi:
-- raylib_ref = "heads/master.zip"
-- raylib_ref = "tags/5.4.zip"

function check_raylib()
	os.chdir("external")
	if os.isdir("raylib-src") == false then
		if not os.isfile("raylib-src.zip") then
			print("Raylib not found, downloading from github")
			local result_str, response_code =
				http.download("https://github.com/raysan5/raylib/archive/refs/" .. raylib_ref, "raylib-src.zip", {
					progress = download_progress,
					headers = { "From: Premake", "Referer: Premake" },
				})
		end
		print("Unzipping to " .. os.getcwd())
		zip.extract("raylib-src.zip", os.getcwd())
		os.remove("raylib-src.zip")

		local extractedDir = nil
		for _, dir in ipairs(os.matchdirs("raylib-*")) do
			extractedDir = dir
			break
		end

		if extractedDir then
			print("Renaming " .. extractedDir .. " to raylib-src")
			os.rename(extractedDir, "raylib-src")
		else
			error("Raylib extraction failed: directory not found")
		end
	end
	os.chdir("../")
end
...
downloadRaylib = false  -- using git submodule
raylib_dir = "../../raylib"

workspaceName = "PatchWall"
...
filter({
	"system:windows",
	"configurations:Release or Release_RGFW",
	"action:gmake*",
})
kind("WindowedApp")
